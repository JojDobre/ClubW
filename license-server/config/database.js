// Umiestnenie: license-server/config/database.js
//
...
// Predvolené hodnoty zodpovedajú licenčný server (vlastná databáza, oddelená od klientskych).
// Sú určené VÝHRADNE pre lokálny vývoj.
const VYVOJOVE_PREDVOLENE = {
  host: 'localhost',
  port: 5435,
  database: 'clubw_licenses_dev',
  username: 'client_dev',
...
const nacitaj = (nazovPremennej, predvolena) => {
  ...
  if (jeProdukcia) {
    throw new Error(
      ...
      `Skopírujte license-server/.env.example do license-server/.env a doplňte údaje.`
    );
  }
