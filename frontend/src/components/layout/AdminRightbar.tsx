...
const NotificationItem: React.FC<NotificationItemProps> = ({ icon, text, time, isNew = false }) => (
  <div className="rightbar-notification-item">
    <div className={`rightbar-notification-icon ${isNew ? 'new' : ''}`}>
      {icon}
    </div>
    <div className="rightbar-notification-content">
      <span className="rightbar-notification-text">{text}</span>
      <span className="rightbar-notification-time">{time}</span>
    </div>
  </div>
);
...
const ActivityItem: React.FC<ActivityItemProps> = ({ avatar, text, time }) => (
  <div className="rightbar-activity-item">
    <div className="rightbar-activity-avatar">
      {avatar}
    </div>
    <div className="rightbar-activity-content">
      <span className="rightbar-activity-text">{text}</span>
      <span className="rightbar-activity-time">{time}</span>
    </div>
  </div>
);
...
const ContactItem: React.FC<ContactItemProps> = ({ avatar, name }) => (
  <div className="rightbar-contact-item">
    <div className="rightbar-contact-avatar">
      {avatar}
    </div>
    <span className="rightbar-contact-name">{name}</span>
  </div>
);

const AdminRightbar: React.FC = () => {
  const { rightbarExpanded } = useLayout();
  ...
  // Avatar components - odstránené, používame importovaný komponent
  
  return (
    <div 
      className={`rightbar ${rightbarExpanded ? 'rightbar-expanded' : 'rightbar-collapsed'}`}
      style={{
        transform: rightbarExpanded ? 'translateX(0)' : 'translateX(100%)',
    ...
    >
      ...
      <div className="rightbar-section">
        ...
        <div className="rightbar-section-content">
          <NotificationItem
            icon={<BugIcon />}
