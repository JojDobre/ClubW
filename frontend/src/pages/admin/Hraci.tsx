...
const STAVY = [
  { hodnota: 'aktivny', popis: tr('Aktívny') },
  { hodnota: 'neaktivny', popis: tr('Neaktívny') },
];
...
const POZICIE = [
  { hodnota: 'brankar', popis: tr('Brankár') },
  { hodnota: 'obranca', popis: tr('Obranca') },
  { hodnota: 'zaloznik', popis: tr('Záložník') },
  { hodnota: 'utocnik', popis: tr('Útočník') },
];
...
export const Hraci: React.FC = () => {
  ...
  const uloz = async () => {
    ...
    if (!upravovany.datum_narodenia) {
      varovanie(tr('Zadajte dátum narodenia'));
      return;
    }
    if (!upravovany.tim_id) {
      varovanie(tr('Vyberte tím'));
      return;
    }
    ...
    try {
      ...
      hlasChybu(e?.message || tr('Hráča sa nepodarilo uložiť'));
    } finally {
      setUklada(false);
    }
  };
  ...
  const zmaz = async () => {
    ...
    try {
      ...
      uspech(tr('Hráč bol presunutý do archívu'));
      setNaZmazanie(null);
      hraci.obnov();
    } catch (e: any) {
      hlasChybu(e?.message || tr('Hráča sa nepodarilo archivovať'));
    } finally {
      setMaze(false);
    }
  };
  ...
  const presunHraca = async () => {
    if (!presuvany) return;
    if (!presun.tim_id) {
      varovanie(tr('Vyberte tím, do ktorého hráča presúvate'));
      return;
    }
    setPresuva(true);
    try {
      await hraciApi.uprav(presuvany.id, { tim_id: presun.tim_id, cislo_dresu: presun.cislo_dresu });
      uspech(tr('{meno} {priezvisko} bol presunutý do tímu {hodnota}', { meno: presuvany.meno, priezvisko: presuvany.priezvisko, hodnota: timPodlaId.get(presun.tim_id)?.nazov ?? '' }));
      setPresuvany(null);
      hraci.obnov();
    } catch (e: any) {
      hlasChybu(e?.message || tr('Presun sa nepodaril'));
    } finally {
      setPresuva(false);
    }
  };
      ...
      obsah: (h) => (
        <div className="cw-hraci__hrac">
          ...
          <div className="cw-hraci__meno">
            ...
            {h.stav === 'neaktivny' && <Badge ton="neutral">{tr('Neaktívny')}</Badge>}
            {/* Príznak z filtrovania osobných údajov detí */}
            {h.fotka_skryta_bez_suhlasu && (
              <span className="cw-hraci__bez-suhlasu" title={tr('Fotka je skrytá — chýba súhlas zákonného zástupcu')}>
                {tr('bez súhlasu')}
              </span>
            )}
          </div>
  ...
  const akcieRiadku: AkciaRiadku<Hrac>[] = [
    { popis: tr('Upraviť'), ikona: 'upravit', onKlik: (h) => otvor(h) },
    { popis: tr('Presunúť do iného tímu'), ikona: 'presunut', onKlik: (h) => otvorPresun(h) },
    {
      popis: tr('Súhlasy a údaje'),
      ikona: 'gdpr',
      onKlik: (h) => navigate(`/admin/ochrana-udajov?hrac=${h.id}`),
    },
    { popis: tr('Archivovať'), ikona: 'archiv', nebezpecna: true, onKlik: (h) => setNaZmazanie(h) },
  ];
  ...
  return (
    <>
      <PageHeader
        nadpis={tr('Hráči')}
        podnadpis={tr('Súpiska všetkých hráčov klubu naprieč tímami.')}
        akcie={
      ...
      <FilterChips
        ...
        popisSkupiny={tr('Filtrovať podľa tímu')}
      />
      ...
      <DataTable<Hrac>
        ...
        hladatPlaceholder={tr('Hľadať podľa mena alebo tímu…')}
        filtre={[
          { kluc: 'pozicia', popis: tr('Všetky pozície'), moznosti: POZICIE },
          { kluc: 'stav', popis: tr('Aktívni aj neaktívni'), moznosti: STAVY },
        ]}
        ...
        prazdnyNadpis={tr('Zatiaľ žiadni hráči')}
        prazdnyPopis={tr('Pridajte hráčov, aby ste mohli zapisovať strelcov a súpisky.')}
        prazdnaAkcia={<Button onClick={() => otvor(PRAZDNY)}>{tr('Pridať prvého hráča')}</Button>}
      />
      ...
      <Modal
        ...
        nadpis={jeNovy ? tr('Nový hráč') : `${upravovany?.meno} ${upravovany?.priezvisko}`}
        pata={
          <>
            ...
            <Button onClick={uloz} nacitava={uklada}>
              {jeNovy ? tr('Pridať hráča') : tr('Uložiť zmeny')}
            </Button>
          </>
        }
      >
          ...
          <>
            ...
            <div className="cw-hraci__row">
              <Input
                ...
                povinne
                napoveda={tr('Pri hráčoch do 18 rokov sa na webe zobrazí len rok')}
              />
              <Input
            ...
            <div className="cw-hraci__row">
              ...
              <Input
                ...
                napoveda={tr('Vyplňte, keď hráč klub opustil')}
              />
            </div>

            <Select
              menovka={jeNovy ? tr('Sezóna (súpiska)') : tr('Zapísať aj na súpisku sezóny')}
              value={
              ...
              prazdna={jeNovy ? tr('Bez sezóny') : tr('Nemeniť')}
              moznosti={(sezony.data ?? [])
                .filter((se) => !se.uzavreta)
                .map((se) => ({ hodnota: se.id, popis: se.nazov }))}
              napoveda={tr('Hráč sa zapíše na súpisku tímu v danej sezóne')}
            />

            <PoleObrazka
              menovka={tr('Fotka hráča')}
              tvar="kruh"
              hodnota={upravovany.fotka}
              onZmena={(cesta) => setUpravovany((d) => ({ ...d!, fotka: cesta }))}
              napoveda={tr('Pri hráčoch do 18 rokov sa fotka zverejní len so súhlasom zákonného zástupcu')}
            />
          </>
        )}
      </Modal>
      ...
      <Modal
        ...
        nadpis={tr('Presunúť: {hodnota} {hodnota2}', { hodnota: presuvany?.meno ?? '', hodnota2: presuvany?.priezvisko ?? '' })}
        podnadpis={presuvany ? tr('Teraz hrá za {hodnota}', { hodnota: timPodlaId.get(presuvany.tim_id)?.nazov ?? '—' }) : undefined}
        sirka="sm"
        pata={
          <>
      ...
      >
        {presuvany && (
          <>
            ...
            />
            <Input
              menovka={tr('Číslo dresu v novom tíme')}
              type="number"
              min={1}
              max={99}
              ...
              napoveda={
                aktualnaSezona
                  ? tr('V súpiske sezóny {nazov} zostane pôvodný tím ako história.', { nazov: aktualnaSezona.nazov })
                  : tr('Súpiska sa upraví, keď bude nastavená aktuálna sezóna.')
              }
            />
          </>
        )}
      ...
      <ConfirmDialog
        ...
        nadpis={tr('Archivovať hráča?')}
        sprava={tr('Hráč {meno} {priezvisko} sa presunie do archívu. Góly a súpisky zostanú zachované a hráča môžete obnoviť. Ak hráč len odišiel, stačí mu nastaviť stav Neaktívny a dátum odchodu.', { meno: naZmazanie?.meno, priezvisko: naZmazanie?.priezvisko })}
